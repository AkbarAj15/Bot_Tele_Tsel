...
# Handler for data penjualan barang
@bot.message_handler(func=lambda message: message.text == "/Data_Penjualan_Barang")
def search_penjualan_barang(message):
    user_id = message.chat.id
    search_status[user_id] = "waiting_for_penjualan_barang"
    # Initialize registration data for the user
    user_search_data[user_id] = {}  # Initialize an empty dictionary
    user_search_data[user_id]["first_name"] = message.chat.first_name
...
@bot.message_handler(func=lambda message: search_status.get(message.from_user.id) == "waiting_for_penjualan_barang")
def get_penjualan_barang(message):
    user_id = message.chat.id
    penjualan_barang = message.text
    user_search_data[user_id]["penjualan_barang"] = penjualan_barang
    search_status[user_id] = "waiting_for_penjualan_barang"
    cursor = db.cursor()
    cursor.execute("SELECT * FROM penjualan WHERE tanggal_penjualan = '{}'".format(penjualan_barang))
    users = cursor.fetchone()
    cursor.close()
    if users is not None:
        retrieved_id_penjualan = users[0]
        retrieved_tanggal_penjualan = users[1]
        retrieved_barang_terjual = users[2]
        retrieved_id_barang = users[3]
        # Adjust this according to your database structure
        # Reply with user information
        reply_message = f"Data Penjualan Barang Tersedia:\nID Penjualan: {retrieved_id_penjualan}\nTanggal Penjualan: {retrieved_tanggal_penjualan}\nBarang Terjual: {retrieved_barang_terjual}\nID Barang: {retrieved_id_barang}"
        bot.reply_to(message, reply_message)
